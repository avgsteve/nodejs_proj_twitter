...
// const Post = require('../../../database/schemas/PostSchema.js');
...
it('測試讀取 profile page GET@/profile && 在server 傳回的 HTML中，檢查是否包括透過 PUG 事先寫入的使用者帳號', async () => {

  const nameToRegister1 = 'steve';
  const responseFromLogin = await signupAndLogin(nameToRegister1);
  // console.log('responseFromLogin  :', responseFromLogin);
  const profilePageResponse = await request(app)
    .get('/profile')
    .set('Cookie', responseFromLogin.get('Set-Cookie'))
    .expect(200);

  // console.log('傳回的HTML:, ', profilePageResponse.text);

  expect(profilePageResponse.text.includes(
    // 在server 傳回的 HTML中，檢查是否包括透過後端API和PUG事先寫入 head title
    `<title>account@${nameToRegister1}</title>`
  ));

  expect(profilePageResponse.text.includes(
    // 在server 傳回的 HTML中，檢查是否包括透過後端API和PUG事先寫入 的全域變數
    `userLoggedIn = {"profilePic":`
  ));

  expect(profilePageResponse.text.includes(
    // 在server 傳回的 HTML中，檢查是否包括透過後端API和PUG事先寫入 的全域變數
    `"userName": ${nameToRegister1}123`
  ));


  expect(profilePageResponse.text.includes(
    // 在server 傳回的 HTML中，檢查是否包括透過後端API和PUG事先寫入 的全域變數 + 使用者id
    `<script>var profileUserId = '${responseFromLogin.body._id}'`
  ));
